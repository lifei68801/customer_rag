...
- **标准名是纯数字的术语跳过检测**——不作为相似度比对的候选，无论是批跑两两比对（`find_duplicate_pairs`）还是创建时点提示（`find_similar_terms`，候选名本身纯数字，或已有术语的标准名纯数字，两种情况都跳过）。这是上线后用真实 `demo` 租户数据验证时发现的真实问题：像"销量"这类术语的标准名是连续整数（100/101/102/...），`longest_common_substring_score` 按最长公共子串/较短字符串长度打分，"100"和"101"共享"10"两个字符，2/3≈0.67，超过0.6阈值——这类术语几乎两两都会命中，几百条数字术语就能刷出上万条毫无意义的审核建议，把队列淹没到不可用。纯数字判定用 `str.strip().isdigit()`，不含小数点/千分位分隔符等场景（这些场景目前没有实际用例，需要时再扩展判定规则）。
