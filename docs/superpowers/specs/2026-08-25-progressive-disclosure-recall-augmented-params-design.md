...
**独立参数生成调用没有自我纠错能力，这是明确接受的代价，不是遗漏。** 在这份设计之前，如果 LLM 直接产出的结构化参数有误，那次失败的 `{"error": ...}` 观察结果会进入 `planner_messages`，同一个模型下一轮能看到自己上次的输出和报错、有机会调整重试。现在独立参数生成调用是无状态、不带历史的单次调用（这是 KV cache 约束和"不携带本轮之前的对话历史"这条 Global Constraint 决定的），它看不到自己上一次生成的参数、也看不到那次的报错——ReAct 推理模型虽然会在下一轮看到这个报错，但它只认识 `query_intent` 这一个字段，不掌握 `anchor`/`constraints`/`term_type` 这些概念（Task 2 收窄 schema 的本意就是让它不需要知道这些），所以它没有词汇去指出"你上次的 term_type 猜错了"，往往只能重复生成几乎相同的 `query_intent`，指望独立调用这次运气好——一旦独立调用真的猜错，很难在预算耗尽前自我纠正，只能依赖 Plan 1 的"轮次耗尽兜底"体面收场，而不是真正修正。

如果未来要补上这个纠错能力，可以在不违反"不带对话历史"这条约束的前提下做：把上一次失败的具体参数和报错原文，作为独立参数生成调用 prompt 里的一段普通上下文（不是对话历史消息）——比如"上次生成的参数：{json}\n它的报错：{error}\n请修正"，`parse_structured_filter_query_args` 目前返回的报错信息（比如"term_type 'X' 不在已确认 schema 里，可用的 term_type: [...]"）本身就相当具体可操作，很可能第一次重试就能修正。这不在本次设计范围内，留给后续单独的计划。
