...
企业产品/SaaS 客服问答机器人。核心是文档知识召回 + 基于知识图谱的专有名词准确率，服务多个租户。**租户是隔离单元，不是「一个客户」的同义词**——见下面 Language 里的定义。

## Language

**租户（tenant）**:
**隔离单元**——`tenant_id` 是这个系统里唯一一条隔离维度，二元的：一处漏过滤就是泄漏。
它圈住的是「什么绝不能被一起查到」：15 张业务表、`neo4j_client.py` 里 199 处过滤、
实体身份 `(tenant_id, node_key)`、检索、会话、记忆、诊断、数据源。跨租户在这个架构里
不是"暂不支持"，是被主动判为缺陷——脏边检测里有一类就叫 `cross_tenant`。
**边界按隔离要求划，不按人设或客户名划**：能一起查且权限相同的领域该合并成一个租户，
权限必须隔离的才分开。见 [ADR-0004](docs/adr/0004-tenant-boundary-follows-isolation-not-persona.md)。
_Avoid_: 把「一个客户」或「一个数字人」直接等同于一个租户——那会让"要几张脸"决定"切几刀"。

**数字人（persona）**:
**展示单元**——前台右栏里那个可切换的对话对象。今天只有三个字段：头像、一句话、
引导问题（`tenant_personas`），对问答行为**零影响**（`_PROMPT_TEMPLATE` 是模块级常量，
没有按租户配的提示词/语气/工具权限）。它是一张脸，不是一个知识边界。
spec D1 曾把它跟租户画等号；在「每个数字人知识本来就不同」的场景下那是同一件事的两个
名字，但当成建租户的方法用就会画错边界——见 ADR-0004。
_Avoid_: 角色、机器人、bot（"角色"跟账号权限里的 role 撞车）
