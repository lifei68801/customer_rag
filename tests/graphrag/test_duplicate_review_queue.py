...
async def test_approve_duplicate_suggestion_merges_via_real_terms_store_and_marks_approved():
    ...
    fake）验证两边接起来确实工作：别名被合并、且审核记录本身被正确标记成
    approved。"""
    ...
    # 检查合并结果：使用合并视图（get_term_merged_by_node_key）来获取最终状态
    keeper = await get_term_merged_by_node_key(conn, "default", coca_cola_node_key)
    assert set(keeper.aliases) == {"coke", "可口可乐", "可乐公司"}

    # 检查 merged 那条在合并视图中不可见（已被 __deleted__ 标记）
    merged_view = await list_terms_merged(conn, tenant_id="default")
    merged_node_keys = {t.node_key for t in merged_view}
    assert cola_node_key not in merged_node_keys

    # 检查 merged 那条的原始行在 terms 表中仍然存在（未被删除）
    all_terms = await list_terms(conn, tenant_id="default")
    merged_term_row = next(t for t in all_terms if t.node_key == cola_node_key)
    # 原始行应该保持不变（没有墓碑化）
    assert merged_term_row.standard_name == "可口可乐"
    assert merged_term_row.aliases == ["可乐公司"]
...
async def test_approve_duplicate_suggestion_propagates_conflict_and_leaves_row_pending():
    """merge_terms() 改到编辑层之后，即使两个术语在 terms 表中共享同一个别名，
    也不会产生冲突异常——编辑层的 aliases 编辑会完全替换 Term 的别名，无需
    冲突检查。验证队列这一层正确处理成功的 merge_terms 调用，并把审核记录
    标记成 approved。"""
    conn = await aiosqlite.connect(":memory:")
    await ensure_terms_schema(conn)
    await ensure_term_edits_schema(conn)
    ...
    # 旁观术语也持有了被合并那条的别名"可乐"——用直接插行来模拟 ETL
    # 写入时可能产生的真实重复别名状态（_check_name_conflict 在那条路径不做检查）。
    await conn.execute(
