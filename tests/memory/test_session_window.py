...
async def test_get_recent_turns_only_returns_matching_session(tmp_path):
    ...
    await append_turn(conn, session_id="s1", user_id="u1", role="user", content="A")
    await append_turn(conn, session_id="s2", user_id="u1", role="user", content="B")
...
async def test_get_recent_turns_respects_limit_keeping_most_recent(tmp_path):
    ...
    turns = await get_recent_turns(conn, session_id="s1", limit=2)
