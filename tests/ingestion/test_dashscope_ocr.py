...
async def test_dashscope_vision_ocr_sends_image_and_prompt_returns_extracted_text(tmp_path):
    ...
    text = await dashscope_vision_ocr(
        image_path,
...
async def test_dashscope_vision_ocr_uses_custom_model_when_provided(tmp_path):
    image_path = tmp_path / "page.png"
    image_path.write_bytes(b"fake-png-bytes")
    captured: dict = {}
...
async def test_build_dashscope_ocr_returns_ocr_function_with_config_baked_in(tmp_path):
