...
后端 FastAPI + LangGraph，前端 React + Vite，附带一套完整的管理后台：本体建模、数据接入、人工审核、问答明细。
...
| **引导建模** | 传一张业务表，平台扫描列分布、推荐一套本体草案；顺带产出该表的 ETL 列映射，不用再配一遍 |
...
`/admin` 会按这个租户走到哪一步分流：本体**未确认**落在「本体结构」，**已确认**落在
「文档上传」。静态选任何一个都必然对一半人是错的——新租户第一屏不该是个主能力被
禁用的页面（本体没确认时文档管线会跳过图谱抽取），而老租户天天来传文档，也不该每
次被丢回建模页。状态还没读到时既不跳转也不空白，停在一个中性的加载态：在不知情时
跳向任何一边，都是对用户断言一件可能为假的事。
...
| **明细查询** | 实体明细 | 按类型分组浏览；大基数类型折叠成摘要行 + 抽样，不逼人逐条看 |
| | 问答明细 | 从一次答错的问答，反查它当时匹配到了哪些实体 |
...
第一次建模不用手写实体类型。「本体结构」页有一个**「从表格开始引导建模」**入口
（它不常驻侧边栏——只在从零到一时用一次，不是流程的一站）：

1. **传一张业务表**（CSV / Excel）。列统计在浏览器里算，不上传到服务端。
2. **平台按列的分布推荐角色**：几乎每行一个值的当标识，重复度高的当分类，带小数的
   当度量。每条推荐都带着依据数字（「1000 个非空值里只有 8 个不同值」），你要能据
   此推翻它。
3. **逐列复核改判**。问的是「你会不会问『某某下面有哪些』」，不是「这该是实体还是
   属性」——后者是建模术语，用户答不了。
4. **写入本体草稿**，同时把这张表的 **ETL 列映射**一并存进草稿。

草稿写完还没生效，去「本体结构」页核对三个 tab 再确认。**确认是不可逆的**：旧的已
确认版本会被换掉。确认框会列出这次要改什么，并对删除实体类型这类破坏性变更附上数
据影响（「当前有 9335 条实体，确认后将不再属于任何已确认类型」）——`confirm` 只动
本体表、不碰 `terms`，那些实体会原地留下但不再属于任何已确认类型，对下游查询不可
见。这一半以前不说，用户只会把「不可逆」理解成「定义会被换掉」。

确认之后去「表格导入」：**映射已经在那儿了**，只要传数据文件。它随本体草稿一起提
升成已确认版本，不用你下载一个 YAML 再传回来——那份映射描述的是「这个本体的实体从
哪张表的哪几列来」，本来就是本体定义的一部分。要接第二张表，或者改这份映射，展开
「改这份映射／再接一张表」；本体已经建好、只是要再接一张表时，那个向导叫「把这张
表映射到已有本体」。

**首次运行建议勾「预演」**：只报告会写入和移除什么，不实际写。看完报告点
**「按这次预演正式执行」**就能转正，复用那次预演已经存在磁盘上的输入，不用重传两
个文件——否则照做的人多传一遍、不照做的人一步到位，推荐的路比不推荐的路更贵，结果
就是没人预演。转正会新建一条运行记录而不是重跑原来那条：预演报告说的是「将会发生
什么」，正式那条说的是「发生了什么」，合并成一条会丢掉前者。

> 转正这条路**不带**「允许大规模清理」——那是安全阀的显式绕过开关，一次点击不该能
> 触发大规模清理。勾了它跑出来的预演，转正时会重新撞阀，界面上会说清这一点。

每一步做完都给下一步的入口：确认成功给「表格导入」，跑批完成给「实体明细」和「待
审关系」。空状态告诉你该先去哪，侧边栏徽标告诉你有活等着，完成点告诉你接下来去哪
——三条出口各管一段。
...
- **ETL 管道**：真相源是干净的关系型表，确定性映射，没有幻觉风险，因此不进审核队列。正确性由映射规则保证——规则对就全对，逐条人工审核没有收益。列映射可以手写 YAML、用「把这张表映射到已有本体」向导配，或者由引导建模从表直接推导出来（见[从零开一个租户](#从零开一个租户)）。
