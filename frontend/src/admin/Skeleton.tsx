...
export function Skeleton({ variant, count = 3 }: SkeletonProps) {
  if (variant === 'table-rows') {
    return (
      <div className="overflow-x-auto border-2 border-ink bg-card shadow-soft-sm" aria-hidden="true">
  ...
  return (
    <div className="flex flex-col gap-2" aria-hidden="true">
      {Array.from({ length: count }, (_, card) => (
        <div key={card} className="flex flex-col gap-2 border-2 border-ink bg-card p-4 shadow-soft-sm">
