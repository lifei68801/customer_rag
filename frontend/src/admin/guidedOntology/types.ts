...
export interface GuidedDecision {
  ...
  rootName?: string
  /**
   * 每个属性列挂在哪个实体下面。键是**原列名**，值是实体名。
   *
   * 缺省（没这一条）= 挂中心。一张订单宽表可能同时带着订单的属性
   * （金额）和客户的属性（客户等级），全挂中心的话，「客户等级」就成了
   * 订单的字段，问「金卡客户有多少」得绕道。
   *
   * 键用原列名不用清洗后的字段名：用户在界面上看到的是原列名，而清洗后
   * 的名字可能被 uniqueFieldName 加过后缀，对不上。
   *
   * 指向的实体如果已经不是实体了（用户后来把它改判成属性），buildProposal
   * 回落到中心——不能把字段挂到一个不在 termTypes 里的名字上。
   *
   * 已知限制：表头重名时（导出的宽表里不罕见）两个同名列共用一个键，改一个
   * 的宿主会把两个一起搬走。它们仍然各自落进某个实体的 extra_fields，不会
   * 丢，只是分不开。真要分开得给列加位置标识，当前没有需求。
   */
  attributeHostOf?: Record<string, string>
}
...
export interface Proposal {
   ...
   * 上级无效、被自动改挂到中心下面的实体。
   *
   * 两种来源：原来的上级已经不在实体明细里（用户把它改判成属性了），
