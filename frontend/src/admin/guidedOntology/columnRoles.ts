 ...
 *
 * 卡的是代价最重的那条出口：判成 identifier 会让这一列直接成为本体的中心，
 * ETL 给它的每一个值建一个节点。工作台的数据面板会把未接住的列连同角色和
 * reason 一起展示，用户可以把它提升为实体或指给已有实体（已命中的列不再
 * 显示 reason），所以这不再是一条静默出口；行数下限仍然留着，因为它挡掉
 * 的是最没有证据的那一类（3 行整数恰好互不相同），少让用户做一次无谓的
 * 纠正。不够行数的整数列落进 freetext，在工作台的数据面板「数据里还有
 * 这些，骨架里没有」一节里可见，可以指给某个实体当字段，但不能提升为
 * 实体（freetext 不在 PROMOTABLE_ROLES 里）。
 */
...
function classify(column: ColumnStats): { role: ColumnRole; reason: string } {
  ...
  // 想改得等草稿建完再去「本体结构」页；而判成 dimension 的列会长出一组
  ...
  // 在两种猜法之间选一种。真正的兜底原来在审阅视图——标识列会连同 reason
  // 一起展示，并且能被改判成属性；那个审阅视图（ProposalReview）随单表引导
  // 建模页一起下线了，建模工作台的「数据」面板（DataPanel）目前没有对应的
  // 展示：「数据里还有这些，骨架里没有」一节只列列名、给"提升为实体类型"
  // 这一个动作，不显示 reason，也没有"改判成属性"的入口。判错时目前只能
  // 等草稿建完后去「本体结构」页手工调整。
  ...
  if (isInteger) {
    ...
    // 动作：建完之后去「本体结构」页手工加成属性（这是个常驻导航项，不依赖
    // 某个建模页面的完成态）。这条 reason 目前没有渲染进建模工作台的任何
