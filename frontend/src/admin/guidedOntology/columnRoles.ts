...
function classify(column: ColumnStats): { role: ColumnRole; reason: string } {
  ...
  // 在两种猜法之间选一种。真正的兜底在建模工作台的「数据」面板（DataPanel）：
  // 没被骨架接住的列连同角色和 reason 一起展示，标识/维度列可以提升为实体
  // 类型，任何列都可以指给某个实体当字段——判成 identifier 的"金额分"在那里
  // 能被指成字段而不是提升成实体。已经按别名命中的列不再显示 reason。
