 ...
 * 「答错了 → 哪个实体不对」这条路上的返回键。
 *
 * 实体详情页此前不管从哪来，返回链接都写死指向实体明细。从问答明细页
 * 点进来的人想回的是那一条问答——回到列表等于把整个调查现场丢了，而且
 * 要重新在几十条问答里找回刚才那一条。
 *
 * 前提是那一条得有地址：问答明细页选中哪条此前只活在组件 state 里，连它
 * 自己刷新一下都回不去。
 */
...
describe('实体详情页的返回键跟着来路走', () => {
  it('从问答明细页进来：回到那一条问答，不是回列表', async () => {
    const user = userEvent.setup()
    renderAt(`${ADMIN_ROUTES.diagnostics}?d=2`)
    ...
    expect(backLink().textContent).toMatch(/问答明细/)
  })

  it('沿着关系再跳一个实体，返回键仍指向那一条问答——链式浏览不该丢掉起点', async () => {
    const user = userEvent.setup()
    renderAt(`${ADMIN_ROUTES.diagnostics}?d=2`)
    await user.click(await screen.findByRole('link', { name: /可口可乐/ }))
    await waitFor(() => expect(screen.getByTestId('term-detail')).toBeTruthy())
  ...
  it('直接打开详情页（没有来路）：回实体明细', async () => {
    ...
    expect(backLink().textContent).toMatch(/实体明细/)
  })
})
