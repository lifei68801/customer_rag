...
from app.graphrag.terms_migrations import apply_terms_migrations
...
async def ensure_terms_schema(
    ...
    if table_already_existed:
        # 整条迁移链在 terms_migrations.py 里，顺序是那个模块的契约（有一步
        # 会重建整张表，重建 DDL 里没有的列必须排在它之后）。这里只调用。
        await apply_terms_migrations(conn)
