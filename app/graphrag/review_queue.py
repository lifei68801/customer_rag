...
async def count_pending_by_reason(
    ...
    只返回**有记录**的那几种，不把没记录的补成 0。

    补 0 要补成什么样，取决于调用方按什么口径分组——`/graph-reviews/counts`
    把五种 reason 归成四个分页，它自己 `sum(by_reason.get(r, 0) ...)` 补出
    每一页的数（那一层必须给全四个 key，缺 key 会让前端写 `?? 0` 兜底，
    而那会把"后端没算这一页"和"这一页真的是 0"混成一件事）。这个函数不知道
    那套分组，在这里补 0 只能按 reason 补，补出来的 key 调用方也用不上。
