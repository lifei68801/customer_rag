...
_USAGE_GUIDE = (
    ...
    "计数问题优先在候选参考的「可能相关的关系」里找 yy 到 xx 的直接一跳关系，"
    "只有确实找不到直连关系时，才退而使用「可能相关的多跳路径」。多跳路径的"
    "计数结果是「经中间类型推导出来的关联数」，不是 xx 名下的精确归属计数："
    "中间类型和 xx 之间如果是多对多（一个中间实体同时关联多个 xx），每个 yy "
    "都会被算进多个 xx 名下，数字会被放大。用多跳路径回答计数问题时必须说明"
    "这是推导出的关联数、不是确定归属，不要当作精确答案给出。\n"
